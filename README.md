...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create kfl-b35a
  fly apps create kfl-b35a-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app kfl-b35a
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app kfl-b35a-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'kfl-b35a-staging' does not match app name in config file 'kfl-b35a'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name kfl-b35a-db
  fly postgres attach --app kfl-b35a kfl-b35a-db

  fly postgres create --name kfl-b35a-staging-db
  fly postgres attach --app kfl-b35a-staging kfl-b35a-staging-db
  ```
...
We use GitHub Actions for continuous integration and deployment. Anything that gets into the `main` branch will be deployed to production after running build/etc. Anything in the `dev` branch will be deployed to staging.
